...
const DUMMY_BCRYPT_HASH = '$2b$12$1w8i2LQyC6z9Yl2wq3FZeu5Vb7J1.2q6oV2Qy1q3bJxJkQe5Lxk1a';
...
export class AuthService {
  ...
  async registerUser(dto: RegisterDto) {
    try {
      ...
      await this.emailService.sendVerificationEmail(user.email, frontendVerifyUrl);

      // Print the verification URL for debugging purposes
      console.log('Verification URL sent to user:', frontendVerifyUrl);

      return { message: 'If this email is valid, a confirmation has been sent.1' };
    } catch (err: any) {
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
      if (err.code === 'P2002') {
        await this.emailService.sendExistingAccountEmail(dto.email);
        return { message: 'If this email is valid, a confirmation has been sent.2' };
      }
      throw err;
    }
  }
  ...
  async confirmEmail(plainToken: string) {
    ...
    if (token.expiresAt.getTime() < Date.now()) throw new ForbiddenException('Token expired');

    await this.userService.enableAccount(token.user.email);
